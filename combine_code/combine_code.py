...
def should_process(path, patterns):
    ...
    for pattern in patterns:
        ...
        if pattern.endswith('/') or pattern.endswith(os.path.sep):
            ...
            # Check if the pattern matches any part of the path exactly
            if any(part == pattern_parts[-1] for part in path_parts):
                if MODE == "blacklist":
                    if DEBUG_MODE:
        ...
        elif fnmatch.fnmatch(relative_path, normalized_pattern) or fnmatch.fnmatch(os.path.basename(relative_path), normalized_pattern):
            if MODE == "blacklist":
                if DEBUG_MODE:
                    print(f"DEBUG: Ignoring {relative_path} because it matches pattern {normalized_pattern}")
                return False
            else:
                if DEBUG_MODE:
                    print(f"DEBUG: Including {relative_path} because it matches pattern {normalized_pattern}")
                return True
...
def combine_files(root_dir, output_file, patterns, run_parameters):
...
def main():
    ...
    combine_files(root_dir, OUTPUT_FILE, patterns, run_parameters)
